...
function pValue(){
	document.getElementById("chkid").value = opener.document.registerform.userid.value;
}
function idcheck(){
	let id = document.getElementById("chkid").value;
	
	if(!id){
		alert("아이디를 입력하지 않았습니다.");
		return false;
	}else if((id < "0" || id >"9") && (id < A || id > Z) && (id < a || id > z)){
		alert("영문과 숫자로만 입력해 주세요.");
		return false;
	}else{
		let param = "id="+id;
...
function sendCheckValue(){
	opener.document.registerform.useridok.value = "idok";
	opener.document.registerform.userid.value = document.getElementById("chkid").value;
	if(opender != null){
		opener.checkFrom = null;
		self.close();
	}
}
function idchk(){
	...
	window.open("member/idcheckform.jsp",
		"chkForm","width=500, height=500, resizeable = no, scrollbars = no");
}
function register(){
    //변수 정의
